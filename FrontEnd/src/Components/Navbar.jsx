...
const Navbar = () => {
  ...
  // Handle opening the third-level submenu

  return (
    <div className="fixed top-0 left-0 right-0 z-50">
      <div className="bg-black h-18 flex items-center px-3 tablet:px-10 flex-wrap mobile:gap-x-0 tablet:gap-x-0 mobile: justify-between  mobile:px-5 ">
        {/* Burger Icon */}
        ...
        {/* Logo */}
        <div className="text-lg font-bold py-4 mobile:w-1/3 tablet:w-1/3  ">
          <img
            src={logo}
            alt="logo"
            className="h-10 tablet:h-12 tablet:w-[22rem]"
          />
        </div>

        {/* Search Input */}
        <div className="relative w-[37%] mobile:order-2 mobile:w-full py-4 mobile:py-2 tablet:order-2 tablet:w-full">
          <input
            type="text"
        ...
        <div className="flex gap-2 mobile:gap-1  items-center mobile:w-1/3  tablet:gap-6 mobile:h-12 tablet:w-1/3">
          <div className="p-4 bg-gray-800 rounded-full hover:bg-gray-700 cursor-pointer   mobile:hidden">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
          ...
          <div className="p-4 bg-gray-800 mobile:bg-black rounded-full mobile:rounded-none hover:bg-gray-700 cursor-pointer">
            <img
              src={fav}
              alt="favorite"
              className="w-6 h-6 mobile:w-[3rem] mobile:h-8"
            />
          </div>
          <div className="p-4 bg-gray-800 mobile:rounded-none mobile:bg-black rounded-full hover:bg-gray-700 cursor-pointer">
            <img
              src={bag}
              alt="bag"
              className="w-6 h-6 mobile:w-[3rem] mobile:h-8"
      ...
      <nav className="hidden lg:flex items-center justify-center w-full text-white bg-gray-800">
        {navdata.map((item, index) => (
          <ul key={index} className="">
            ...
            <div
              className={`flex w-full bg-white   z-10 absolute top-50 left-0 right-0 gap-5  justify-center flex-wrap  space-x-4 transition-opacity duration-300 ${
                isUnderNav ? "opacity-100" : "opacity-0 lg:hidden"
              }`}
              onMouseEnter={() => {
                showUnderNav();
              }}
            ...
            >
              {selectedItem.subCategories &&
                selectedItem.subCategories.map((item, index) => (
                  <section className=" p-4 font-bold w-auto  text-black ">
                    {item.name}
                    <section>
                      {item.subCategories &&
                        item.subCategories.map((item, index) => (
                          <section className="font-normal text-sm text-black mt-2">
                            {item.name}
                          </section>
                        ))}
                    </section>
      ...
      {/* Navbar for Mobile */}
      <nav
        className={`lg:hidden fixed top-30 left-0 z-50 bg-white w-full p-2  pb-4 overflow-y-auto transform transition-transform duration-300 ease-in-out ${
      ...
      {/* Submenu for Mobile */}
      <nav
        className={`lg:hidden fixed top-30 right-0 z-50 bg-white w-full  overflow-y-auto transform transition-transform duration-300 ease-in-out ${
          isOpen2 ? "translate-x-100" : "translate-x-full "
        }`}
      ...
      <nav
        className={`lg:hidden fixed top-30 right-0 z-50 bg-white w-full overflow-y-auto transform transition-transform duration-300 ease-in-out ${
          isOpen3 ? "translate-x-100" : "translate-x-full "
        }`}
        style={{ maxHeight: "80vh" }}
      >
            ...
            selectedDropdown2.subCategories.map((item, index) => (
              <li key={index} className=" group w-full relative">
                ...
                >
                  ...
                  <span className="  font-bold text-black text-xl absolute right-6"></span>
                </a>
              </li>
            ))}
        </ul>
